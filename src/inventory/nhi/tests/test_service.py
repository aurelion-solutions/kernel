...
async def test_create_nhi_emits_inventory_nhi_created(
    ...
    assert envelope.payload['nhi_id'] == str(nhi.id)
    assert envelope.payload['external_id'] == nhi.external_id
    assert envelope.payload['subject_type'] == 'nhi'
    # subject_ref is Subject.id — must differ from nhi.id
    assert 'subject_ref' in envelope.payload
    assert envelope.payload['subject_ref'] != str(nhi.id)
...
@pytest.mark.asyncio
async def test_create_nhi_creates_subject(
    event_service: EventService,
    session_factory,
) -> None:
    """create_nhi leaves exactly one Subject for the new NHI."""
    ...
    svc = NHIService(event_service=event_service, subject_service=subject_service)

    async with session_factory() as session:
        nhi = await svc.create_nhi(
            session,
            external_id='nhi-subj-create',
            name='NHI Subj Create',
        ...
        count = (
            await session.execute(
                sa.select(sa.func.count()).where(
                    ...
                    Subject.principal_nhi_id == nhi.id,
                )
            )
        ).scalar()
    assert count == 1
