...
"""CapabilityGrant service — projection writer and read-only query driver.

CapabilityProjectionService:
- project_for_effective_grant: loads one EG, projects against active mappings, upserts.
- project_for_application: iterates all live EGs for an application, projects, upserts.
- tombstone_for_effective_grant: soft-deletes all grants sourced from an EG.

CapabilityGrantReadService:
- list_grants / get_grant: pass-through read queries.

Session discipline (per ARCH_CONTEXT):
- Services flush. Callers commit. No commit/rollback here.

No events, no LogService — event emission is the ScanEngine's job in Step 14.

Forbidden in CapabilityGrantReadService: emit_safe, emit_log, session.flush,
...
from src.capabilities.access_analysis.capability_grants import capability_projector
...
from src.capabilities.access_analysis.capability_grants.mapping_loader import load_active_mappings
...
from src.capabilities.access_analysis.capability_grants.schemas import ProjectionRunSummary
from src.capabilities.access_analysis.capability_mappings.service import resolve_default_scope_key_id
from src.capabilities.effective_access.models import EffectiveGrant
from src.capabilities.effective_access.repository import get_effective_grant
...
async def _load_resource(session: AsyncSession, resource_id: UUID) -> Resource:
    """Fetch a Resource by id. Raises ValueError if missing."""
...
async def _load_resource_attributes(session: AsyncSession, resource_id: UUID) -> dict[str, str]:
    """Load resource attributes as a key→value dict. Returns empty dict if none."""
    # resource_attributes table may or may not exist. Use text query for resilience.
    try:
        ...
        # Table may not exist yet in this phase. Return empty dict.
        return {}
...
async def _build_grant_view(
    ...
    """Build EffectiveGrantView from an EffectiveGrant ORM row."""
    ...
    # Subject attributes: future-phase concept, no store yet.
...
def _accumulate(total: UpsertResult, batch: UpsertResult) -> UpsertResult:
    """Accumulate batch UpsertResult into total."""
    return UpsertResult(
...
class CapabilityProjectionService:
    """Projection driver for CapabilityGrant.
    ...
    async def project_for_application(
        ...
        *,
        ...
        while True:
            from src.capabilities.effective_access.models import EffectiveGrant as EG

            base_stmt = sa.select(EG).where(EG.application_id == application_id)
            if last_id is None:
                batch_stmt = base_stmt.order_by(EG.id).limit(_BATCH_SIZE)
            else:
                batch_stmt = base_stmt.where(EG.id > last_id).order_by(EG.id).limit(_BATCH_SIZE)
