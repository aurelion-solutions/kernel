...
async def get_active_apply_run_for_reconciliation(
    ...
    """Return an active (running | completed | partially_applied) apply run for a reconciliation run.

    Used by idempotency check in ``SyncApplyService.apply``.
    """
    active_statuses = [
...
async def update_reconciliation_run_status(
    ...
    *,
    ...
    Flushes but does not commit.
    """
    from src.capabilities.reconciliation.models import ReconciliationRunStatus
