...
@pytest.mark.asyncio
async def test_effective_grant_fk_rejects_unknown_source_access_fact_id(session_factory) -> None:
    """Inserting with a random source_access_fact_id raises IntegrityError 23503."""
    async with session_factory() as session:
        subject_id = await _make_employee_subject(session)
        app_id, resource_id = await _make_application_and_resource(session)
        fact_id = await _make_access_fact(session, subject_id, resource_id)
        ...
        grant = _grant(
            ...
            application_id=app_id,
            resource_id=resource_id,
            source_access_fact_id=uuid.uuid4(),  # does not exist
            source_initiative_id=initiative_id,
        )
        session.add(grant)
        with pytest.raises(IntegrityError) as exc_info:
