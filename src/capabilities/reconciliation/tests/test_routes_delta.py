...
"""API tests: POST /reconciliation/runs + GET /reconciliation/runs/{id}/delta-items."""
...
# ---------------------------------------------------------------------------
# POST /reconciliation/runs tests
# ---------------------------------------------------------------------------
...
async def test_post_run_mode_review_returns_200(session_factory):
    ...
    with patch(
        'src.capabilities.reconciliation.routes.get_run',
...
async def test_run_auto_apply_runs_pipeline_then_apply(session_factory):
    ...
    from src.capabilities.reconciliation.models import ReconciliationRun, ReconciliationRunStatus
...
# ---------------------------------------------------------------------------
# GET /reconciliation/runs/{id} tests
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_get_run_returns_200(session_factory):
    """GET /reconciliation/runs/{id} → 200 with run body."""
    run_id = uuid4()
    app_id = uuid4()
    ...
    with patch(
        ...
        async with AsyncClient(
            ...
            resp = await client.get(f'/reconciliation/runs/{run_id}')
...
async def test_get_run_unknown_id_returns_404(session_factory):
    """GET /reconciliation/runs/{id} with unknown id → 404."""
    run_id = uuid4()
...
# ---------------------------------------------------------------------------
# GET /reconciliation/runs/{id}/delta-items tests
# ---------------------------------------------------------------------------
...
async def test_get_delta_items_returns_200(session_factory):
    ...
    with (
        ...
        patch(
            'src.capabilities.reconciliation.routes.list_delta_items',
        ...
        async with AsyncClient(
            ...
            resp = await client.get(f'/reconciliation/runs/{run_id}/delta-items')
    ...
    with (
        ...
        async with AsyncClient(
            ...
            resp = await client.get(
                f'/reconciliation/runs/{run_id}/delta-items',
                params={'limit': limit},
            )
