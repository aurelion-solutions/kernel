...
"""End-to-end integration test: lake-only pipeline.
...
@pytest.mark.asyncio
async def test_lake_only_pipeline(client_iceberg: Any, session_factory: Any, app_iceberg: Any) -> None:
    """Full lake-only pipeline: bulk ingest → reconciliation → apply → scan."""
    ...
    event_types_after_recon = get_event_types(app_iceberg.state.event_buffer)
    ...
    event_types_after_apply = get_event_types(app_iceberg.state.event_buffer)
    ...
    _verify_facts_in_iceberg(app_iceberg, expected)

    # -----------------------------------------------------------------------
    # Stage 5 — Scan run: create + execute
    # -----------------------------------------------------------------------
    scan_create_resp = await client_iceberg.post(
        '/api/v0/scan-runs',
    ...
    scan_run_resp = await client_iceberg.post(f'/api/v0/scan-runs/{scan_run_id}/run')
    ...
    event_types_final = get_event_types(app_iceberg.state.event_buffer)
