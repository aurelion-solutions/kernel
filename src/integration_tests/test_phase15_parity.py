...
"""Phase 15 Step 19 — env-gated parity smoke test.

Skipped by default (no env gate set). Run locally or in a dedicated nightly job by
setting ``PHASE15_PARITY_DATASET=1`` (or any non-empty value).
...
pytestmark = pytest.mark.skipif(
    os.getenv('PHASE15_PARITY_DATASET') is None,
    reason='PHASE15_PARITY_DATASET env var not set — parity test skipped',
)
...
@pytest.mark.asyncio
async def test_phase15_parity_matches_golden_fixture(
    client_p15: Any,
    session_factory: Any,
    app_p15: Any,
) -> None:
    ...
    lake_settings = app_p15.state.test_lake_settings
    warehouse_uri = lake_settings.warehouse_uri
    factory = app_p15.state.lake_session_factory
...
def _assert_parity_invariants(
    ...
    if failures:
        ...
        pytest.fail(f'Phase 15 parity invariants violated ({len(failures)} failure(s)):\n{diff_lines}')
