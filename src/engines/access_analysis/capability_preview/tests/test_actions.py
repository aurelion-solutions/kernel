...
"""Tests for access_analysis.capability_preview engine actions.

Covers:
- Registry presence for resolve action.
...
_ENGINE = 'access_analysis.capability_preview'
_ACTION = 'resolve'
...
_ACTIONS_MODULE = 'src.engines.access_analysis.capability_preview.actions'
...
def test_action_is_registered() -> None:
    """capability_preview.resolve action is registered with correct metadata."""
...
@pytest.mark.asyncio
async def test_dispatch_resolve_empty_sources_returns_result(session_factory) -> None:  # type: ignore[no-untyped-def]
    """Dispatch resolve with empty sources list returns empty capability_slugs."""
    async with session_factory() as session:
        ...
        assert hasattr(validated, 'capability_slugs')
        assert validated.capability_slugs == []
