...
async def _make_employee_subject(session) -> uuid.UUID:  # type: ignore[no-untyped-def]
    ...
    person = await create_person(session, external_id=str(uuid.uuid4()), full_name='e2e test person')
    await session.flush()
...
async def test_phase_13_end_to_end(client, session_factory) -> None:
    ...
    mitigation_valid_until = None  # no expiry — avoids test breaking when run after _NOW date
