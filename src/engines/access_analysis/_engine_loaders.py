...
from src.engines.policy_assessment.policy_types.sod.repository import load_enabled_rules
...
# ---------------------------------------------------------------------------
# Privileged-access inputs
# ---------------------------------------------------------------------------


async def load_privileged_inputs(
    session: AsyncSession,
    *,
    scope_subject_id: UUID | None = None,
    scope_application_id: UUID | None = None,
) -> list[PrivilegedCandidateView]:
    """Load active EffectiveGrant rows joined to Account and Resource.

    Filter: ``effect = allow`` and ``tombstoned_at IS NULL``. Rows where
    ``account_id`` is NULL are still returned — ``account_is_privileged``
    falls back to ``False`` because there is no account to flag.

    Single round-trip. The cartridge filters the privileged candidates from
    this superset; the loader does not pre-classify.
    """
    stmt = (
        sa.select(
            EffectiveGrant.id.label('effective_grant_id'),
            EffectiveGrant.subject_id,
            EffectiveGrant.account_id,
            EffectiveGrant.application_id,
            EffectiveGrant.resource_id,
            EffectiveGrant.action,
            sa.func.coalesce(Account.is_privileged, sa.false()).label('account_is_privileged'),
            Resource.privilege_level.label('resource_privilege_level'),
            Resource.environment.label('resource_environment'),
            Resource.data_sensitivity.label('resource_data_sensitivity'),
        )
        .outerjoin(Account, Account.id == EffectiveGrant.account_id)
        .join(Resource, Resource.id == EffectiveGrant.resource_id)
        .where(
            EffectiveGrant.effect == 'allow',
            EffectiveGrant.tombstoned_at.is_(None),
        )
        .order_by(EffectiveGrant.id)
    )
    if scope_subject_id is not None:
        stmt = stmt.where(EffectiveGrant.subject_id == scope_subject_id)
    if scope_application_id is not None:
        stmt = stmt.where(EffectiveGrant.application_id == scope_application_id)
    ...
    return [
        PrivilegedCandidateView(
            effective_grant_id=row.effective_grant_id,
            subject_id=row.subject_id,
            account_id=row.account_id,
            application_id=row.application_id,
            resource_id=row.resource_id,
            action=row.action.value if hasattr(row.action, 'value') else str(row.action),
            account_is_privileged=bool(row.account_is_privileged),
            resource_privilege_level=(
                row.resource_privilege_level.value
                if hasattr(row.resource_privilege_level, 'value')
                else row.resource_privilege_level
            ),
            resource_environment=(
                row.resource_environment.value
                if hasattr(row.resource_environment, 'value')
                else row.resource_environment
            ),
            resource_data_sensitivity=(
                row.resource_data_sensitivity.value
                if hasattr(row.resource_data_sensitivity, 'value')
                else row.resource_data_sensitivity
            ),
        )
        for row in rows
    ]
