...
from src.engines.reconciliation.registry import _reset_registry_for_tests  # noqa: E402
from src.engines.reconciliation.service import ReconciliationService  # noqa: E402
...
def _register_all_handlers() -> None:
    ...
    from src.engines.reconciliation.handlers.acl_entry import AclEntryHandler
    from src.engines.reconciliation.handlers.db_grant import DbGrantHandler
    ...
    from src.engines.reconciliation.handlers.sap_role import SapRoleHandler
...
async def _make_subject(session) -> uuid.UUID:
    ...
    person = await create_person(session, external_id=str(uuid.uuid4()), full_name='e2e-test')
    await session.flush()
