...
def compute_natural_key_hash(
    ...
    Args:
        ...
        subject_id:  Subject UUID, or ``None`` when account owner is unknown;
                     encoded as ``\\x00`` when absent.
    ...
    """
    subject_part = str(subject_id) if subject_id is not None else '\x00'
    account_part = str(account_id) if account_id is not None else '\x00'
    raw = f'{app_id}|{subject_part}|{account_part}|{resource_id}|{action_id}|{effect}'
