...
async def _seed_capability_mapping(session, cap_id: int, sk_id: int, app_id: uuid.UUID) -> int:  # type: ignore[no-untyped-def]
    m = CapabilityMapping(
...
async def _seed_eg_chain(
    ...
    """Seed Initiative → EffectiveGrant. Returns (eg_id, fact_id, initiative_id)."""
    fact_id = uuid.uuid4()
    ...
    session.add(eg)
    await session.flush()
    return eg.id, fact_id, initiative.id
...
async def test_evaluate_subject_happy_path(session_factory) -> None:  # type: ignore[no-untyped-def]
    ...
    async with session_factory() as session:
        ...
        mapping_id_1 = await _seed_capability_mapping(session, cap_id_1, sk_id, app_id)
        mapping_id_2 = await _seed_capability_mapping(session, cap_id_2, sk_id, app_id)
        await _seed_capability_grant(session, subject_id, cap_id_1, sk_id, app_id, eg_id_1, mapping_id_1, _NOW)
        await _seed_capability_grant(session, subject_id, cap_id_2, sk_id, app_id, eg_id_2, mapping_id_2, _NOW)
...
async def test_active_at_filtering_excludes_tombstoned(session_factory) -> None:  # type: ignore[no-untyped-def]
    ...
    async with session_factory() as session:
        ...
        await _seed_capability_grant(
            session, subject_id, cap_id_1, sk_id, app_id, eg_id_1, mapping_id_1, _START, tombstoned_at=_TOMB
        )
        # Grant 2 is active
        await _seed_capability_grant(session, subject_id, cap_id_2, sk_id, app_id, eg_id_2, mapping_id_2, _START)
...
async def test_mitigation_validity_window_filtering(session_factory) -> None:  # type: ignore[no-untyped-def]
    ...
    async with session_factory() as session:
        ...
        mapping_id_1 = await _seed_capability_mapping(session, cap_id_1, sk_id, app_id)
        mapping_id_2 = await _seed_capability_mapping(session, cap_id_2, sk_id, app_id)
        await _seed_capability_grant(session, subject_id, cap_id_1, sk_id, app_id, eg_id_1, mapping_id_1, _GRANT_AT)
        await _seed_capability_grant(session, subject_id, cap_id_2, sk_id, app_id, eg_id_2, mapping_id_2, _GRANT_AT)
