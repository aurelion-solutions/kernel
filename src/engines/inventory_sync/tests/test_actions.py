...
from src.engines.inventory_sync.actions import SyncApplyApplyArgs, SyncApplyApplyResult
from src.engines.inventory_sync.lake_writer import PreflightRecoveryResult
...
async def test_apply_dry_run_via_registry(session_factory) -> None:  # type: ignore[no-untyped-def]
    ...
    async with session_factory() as session:
        ...
        with (
            patch(
                'src.engines.inventory_sync.actions.get_process_lake_session',
                new=AsyncMock(return_value=fake_lake_session),
            ),
            patch(
                'src.engines.inventory_sync.actions.get_process_lake_catalog',
            ...
            patch(
                'src.engines.inventory_sync.service.preflight_recover_already_written',
...
async def test_apply_auto_apply_emits_event(session_factory) -> None:  # type: ignore[no-untyped-def]
    ...
    from src.engines.inventory_sync.lake_writer import RunWriteResult  # noqa: PLC0415
    ...
    async with session_factory() as session:
        ...
        with (
            ...
            patch(
                'src.engines.inventory_sync.service.write_run_batch',
