...
"""Tests for src/engines/inventory_sync/lake_writer.py.
...
def test_lake_writer_imported_only_from_sync_apply() -> None:
    ...
    # Both inventory_sync (post-rename) and sync_apply (legacy/shim) may own lake_writer.
    allowed_paths = (
        str(kernel_src / 'engines' / 'inventory_sync'),
        str(kernel_src / 'engines' / 'sync_apply'),
    )
    for filepath in hitting_files:
        assert any(filepath.startswith(p) for p in allowed_paths), (
            f'lake_writer imported outside sync_apply/ or inventory_sync/: {filepath}\n'
            f'All importing files: {hitting_files}'
        )
