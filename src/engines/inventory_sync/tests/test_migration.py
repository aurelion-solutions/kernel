...
def _sync_drop_sync_apply_schema(sync_conn: Any) -> None:
    ...
    # Drop child before parent (FK order) — Phase 15 migration creates sync_apply_* names
...
async def migration_engine() -> AsyncEngine:  # type: ignore[misc]
    ...
    import src.engines.inventory_reconcile.models  # noqa: F401, PLC0415
    import src.engines.inventory_sync.models  # noqa: F401, PLC0415
    from src.engines.inventory_sync.models import SyncApplyResult, SyncApplyRun  # noqa: PLC0415
    ...
    async with engine.begin() as conn:
        ...
        # Phase 15 Step 10 migration uses pre-A2 table names ('reconciliation_runs',
        # 'reconciliation_delta_items') as FK targets.  After A2 those are
        # 'inventory_reconcile_runs' / 'inventory_reconcile_delta_items'.
        # Provide stub tables so the migration FK references resolve.
        await conn.execute(sa.text('CREATE TABLE IF NOT EXISTS reconciliation_runs (id UUID PRIMARY KEY)'))
        await conn.execute(
            sa.text(
                'CREATE TABLE IF NOT EXISTS reconciliation_delta_items '
                '(id UUID PRIMARY KEY, reconciliation_run_id UUID NOT NULL '
