...
_run_status_enum = SaEnum(
    SyncApplyRunStatus,
    name='inventory_sync_run_status',
    create_type=True,
...
_result_status_enum = SaEnum(
    ...
    name='inventory_sync_result_status',
    create_type=True,
    values_callable=lambda x: [e.value for e in x],
)
...
class SyncApplyRun(Base):
    ...
    reconciliation_run_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        sa.ForeignKey('inventory_reconcile_runs.id', ondelete='RESTRICT'),
        nullable=False,
    )
    ...
    __table_args__ = (
        sa.Index('ix_inventory_sync_runs_reconciliation_run_id', 'reconciliation_run_id'),
        sa.Index('ix_inventory_sync_runs_status', 'status'),
    )
...
class SyncApplyResult(Base):
    ...
    __tablename__ = 'inventory_sync_results'
    ...
    sync_apply_run_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        sa.ForeignKey('inventory_sync_runs.id', ondelete='CASCADE'),
        nullable=False,
    )
    delta_item_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        sa.ForeignKey('inventory_reconcile_delta_items.id', ondelete='RESTRICT'),
        nullable=False,
    )
    ...
    __table_args__ = (
        ...
        sa.Index('ix_inventory_sync_results_delta_item_id', 'delta_item_id'),
    )
