...
from src.engines.inventory_sync.repository import (
...
from src.engines.inventory_sync.schemas import FactDescriptor, SingleFactSyncOp, SyncApplyApplyResponse
...
class SyncApplyService:
    ...
    - Manual apply requests (``POST /inventory-reconciles/runs/{id}/apply``) arrive on a
    ...
    def sync_single_fact(
        ...
        *,
        ...
        """Write one access-fact event to ``normalized.access_facts`` with wire-level idempotency.

        This is the F2 single-fact sync primitive used by the F3 post-apply chain in
        ``access_apply``.  It is intentionally synchronous — DuckDB scan and PyIceberg
        append are blocking I/O; callers should wrap in ``asyncio.to_thread`` if needed.

        Protocol:
        1. Preflight DuckDB scan: if a row with ``event_key`` already exists → return False
           (skip append, wire-level idempotency).
        2. Otherwise → append one row via :func:`append_single_fact_row` with the given
           ``event_key``.  Returns True.

        ``op=grant`` → ``is_active=True``, ``effect='allow'``.
        ``op=revoke`` → ``is_active=False``, ``effect='deny'``.

        The ``descriptor`` parameter carries fact metadata (kind, application, target).
        Subject/resource/action IDs are passed separately because the descriptor is
        connector-agnostic and does not embed kernel-internal UUIDs.

        Args:
            descriptor: Fact descriptor (kind, application, target_descriptor).
            op: ``grant`` or ``revoke``.
            event_key: Deterministic idempotency key (e.g. ``hash(plan_item_id, op)``).
            subject_id: Subject UUID.
            resource_id: Resource UUID.
            action_id: Action identifier string.
            application_id_denorm: Application UUID string (partition field).
            subject_kind_denorm: Subject kind string (partition field, e.g. ``employee``).
            account_id: Optional account UUID.
            correlation_id: Optional correlation ID for observability.

        Returns:
            True if the row was appended; False if it was skipped (already exists).
        """
        cid = correlation_id or ''

        # --- Step 1: preflight — wire-level idempotency check ---
        if check_event_key_exists(event_key, lake_session=self._lake_session):
            # allowed-emit-safe: observability
            self._logs.emit_safe(
                level=LogLevel.INFO,
                message='inventory_sync.single_fact_skipped',
                component=_COMPONENT,
                payload=merge_emit_component_trace_fields(
                    {
                        'event_key': event_key,
        ...
        # --- Step 2: build row and append ---
        is_active = op == SingleFactSyncOp.grant
        ...
        row = SingleFactRow(
            ...
            subject_kind_denorm=subject_kind_denorm,
            event_key=event_key,
            account_id=account_id,
            observed_at=now,
        )

        append_single_fact_row(row, catalog=self._catalog, log_service=self._logs)  # type: ignore[arg-type]

        # allowed-emit-safe: observability
        self._logs.emit_safe(
            level=LogLevel.INFO,
            message='inventory_sync.single_fact_written',
            component=_COMPONENT,
            payload=merge_emit_component_trace_fields(
                {
                    'event_key': event_key,
