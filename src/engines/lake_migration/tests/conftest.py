...
def _resolve_db_url() -> str:
    try:
        ...
        pass
    return os.getenv('DATABASE_URL', 'postgresql+asyncpg://localhost/aurelion')
...
async def engine():
    ...
    import src.engines.lake_migration.models  # noqa: F401
    import src.engines.reconciliation.models  # noqa: F401
    import src.engines.sync_apply.models  # noqa: F401

    # Phase 15 Step 16: src.inventory.access_artifacts.models and access_facts.models
    # were deleted — both now live in Iceberg.
    ...
    async with eng.begin() as conn:
        # Drop legacy access_artifacts/access_facts (raw SQL — not in Base.metadata)
        ...
        # Phase 15 Step 16 dropped access_artifacts and access_facts from PG.
        # The lake_migration service migrates legacy data PG → Iceberg, so tests
        # need these tables present to seed pre-migration state.
        # Schema mirrors the Step-16 migration's downgrade(), simplified to columns
        # the lake_migration service reads/writes.
        await conn.execute(
            sa.text(
