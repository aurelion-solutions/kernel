...
from src.engines.effective_access.projector import EffectiveGrantDraft
from src.engines.effective_access.repository import upsert_effective_grants
from src.engines.effective_access.schemas import IncrementalApplyKind, ProjectionScopeKind
from src.engines.effective_access.service import (
...
async def test_project_access_fact_event_emission(session_factory) -> None:
    async with session_factory() as session:
        ...
        assert envelope.actor_id == 'engines.effective_access'
...
async def test_project_access_fact_no_event_on_exception(session_factory) -> None:
    async with session_factory() as session:
        ...
        with patch(
            'src.engines.effective_access.service.upsert_effective_grants',
...
async def test_project_access_fact_emits_eas_projection_failed_on_pair_mismatch(session_factory) -> None:
    ...
    async with session_factory() as session:
        ...
        with patch(
            'src.engines.effective_access.service.project',
