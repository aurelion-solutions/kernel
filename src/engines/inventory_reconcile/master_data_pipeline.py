...
async def run_accounts_reconciliation(
    session: AsyncSession,
    lake_session: LakeSession,
    *,
    run: ReconciliationRun,
) -> MasterDataReconciliationResult:
    """Reconcile raw.accounts → PG ent_accounts table.

    Key:        (application_id, username)
    Comparison: display_name, email, is_privileged, mfa_enabled, status
    Status transitions: active→disabled = revoke, disabled→active = reactivate.
    """
    from src.engines.inventory_reconcile.handlers.account import AccountHandler  # noqa: PLC0415
    from src.inventory.accounts.models import Account, AccountStatus  # noqa: PLC0415

    run_id = run.id
    handler = AccountHandler()

    # Phase 1: load raw lake data (latest active rows per natural key).
    table_path = lake_session.iceberg_table_path('raw', 'accounts')
    sql = (
        f'SELECT application_id, username, external_id, display_name, email, '
        f'status, is_privileged, mfa_enabled, meta, observed_at '
        f"FROM iceberg_scan('{table_path}') WHERE is_active = true"
    )
    lake_rows = _fetch_all_batched(lake_session, sql, [])

    # {(application_id, username): row_dict}
    lake_map: dict[tuple[str, str], dict[str, Any]] = {}
    for row in lake_rows:
        ...
        lake_map[key] = {
            ...
            'display_name': row[3],
            'email': row[4],
            'status': str(row[5]) if row[5] is not None else AccountStatus.active,
            'is_privileged': bool(row[6]) if row[6] is not None else False,
            'mfa_enabled': bool(row[7]) if row[7] is not None else False,
            'meta': row[8],
            'observed_at': row[9],
        }
    ...
    pg_rows = result.all()
    # {(application_id_str, username): Account-like dict with id}
    pg_map: dict[tuple[str, str], dict[str, Any]] = {
        (str(row.application_id), row.username): {
            'id': row.id,
    ...
    # Phase 4: compute delta using AccountHandler
    from datetime import UTC, datetime  # noqa: PLC0415

    from src.engines.inventory_reconcile.handlers.account import ObservedAccount  # noqa: PLC0415
    ...
    for key in lake_keys:
        lake_row = normalized[key]
        # Build an ObservedAccount from lake data
        observed_at_val = lake_row['observed_at']
        if hasattr(observed_at_val, 'as_py'):
            observed_at_val = observed_at_val.as_py()
        if observed_at_val is None:
            observed_at_val = datetime.now(UTC)

        observed = ObservedAccount(
            application_id=lake_row['application_id'],
            username=lake_row['username'],
            external_id=lake_row.get('external_id'),
            display_name=lake_row.get('display_name'),
            email=lake_row.get('email'),
            status=lake_row.get('status') or 'active',
            is_privileged=bool(lake_row.get('is_privileged', False)),
            mfa_enabled=bool(lake_row.get('mfa_enabled', False)),
        ...
        if pg_row is not None:
            # Reconstruct a minimal Account-like ORM object for the handler
            existing = Account(
                id=pg_row['id'],
                application_id=pg_row['application_id'],
                username=pg_row['username'],
                display_name=pg_row['display_name'],
                email=pg_row['email'],
                status=AccountStatus(str(pg_row['status'])) if pg_row['status'] else AccountStatus.active,
                is_privileged=pg_row['is_privileged'],
                mfa_enabled=pg_row['mfa_enabled'],
            )

        delta_item = handler.compute_delta(
            reconciliation_run_id=run_id,
            observed=observed,
            existing=existing,
        )

        if delta_item.operation == ReconciliationDeltaOperation.create:
            created_count += 1
        elif delta_item.operation == ReconciliationDeltaOperation.update:
            updated_count += 1
        elif delta_item.operation == ReconciliationDeltaOperation.revoke:
            revoked_count += 1
        elif delta_item.operation == ReconciliationDeltaOperation.reactivate:
            updated_count += 1
        elif delta_item.operation == ReconciliationDeltaOperation.noop:
            unchanged_count += 1

        # Persist all items including noop for audit trail
        items.append(delta_item)

    # Accounts in PG but not in lake → revoke
    for key in pg_keys - lake_keys:
        pg_row = pg_map[key]
        existing = Account(
            id=pg_row['id'],
    ...
    # Phase 5: persist delta (skip noop items to keep delta table clean)
    non_noop = [it for it in items if it.operation != ReconciliationDeltaOperation.noop]
    if non_noop:
        await bulk_insert_delta_items(session, non_noop)

    # Adjust unchanged_count: noop items ARE unchanged
    return MasterDataReconciliationResult(
        run_id=run_id,
        entity_type=ReconciliationEntityType.account,
        created_count=created_count,
...
async def run_master_data_reconciliation(
    ...
    *,
    ...
    the apply step via POST /inventory-reconciles/master-data/runs/{id}/apply.
    """
    from src.engines.inventory_reconcile.repository import create_run  # noqa: PLC0415
    ...
    try:
        dispatch = {
            ...
            ReconciliationEntityType.account: run_accounts_reconciliation,
        }
