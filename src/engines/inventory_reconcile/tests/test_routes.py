...
"""Integration tests for POST /inventory-reconciles/runs (legacy test_routes.py, updated Step 9)."""
...
async def test_post_run_happy_path(session_factory):
    """POST /inventory-reconciles/runs returns 200 + ReconciliationRunRead JSON."""
    app_id = uuid.uuid4()
    run_id = uuid.uuid4()
...
async def test_post_run_application_not_found_404(session_factory):
    """POST /inventory-reconciles/runs with unknown application_id → 404."""
...
async def test_post_run_unknown_application_id_returns_404_real_db(session_factory):
    """POST /inventory-reconciles/runs with random UUID → 404 via real DB path (no service mock).
