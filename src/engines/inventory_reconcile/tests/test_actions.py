...
_ENGINE = 'inventory_reconcile'
_ACTIONS_MODULE = 'src.engines.inventory_reconcile.actions'
...
async def test_master_data_apply_happy_path(session_factory) -> None:  # type: ignore[no-untyped-def]
    ...
    async with session_factory() as session:
        ...
        from src.engines.inventory_reconcile.models import ReconciliationDeltaItem as _DI  # noqa: PLC0415
...
async def test_run_happy_path_mocked_lake(session_factory) -> None:  # type: ignore[no-untyped-def]
    ...
    from src.engines.inventory_reconcile.schemas import ReconciliationRunSummary  # noqa: PLC0415
    ...
    async with session_factory() as session:
        ...
        with (
            patch(
                'src.engines.inventory_reconcile.actions.get_process_lake_session',
                new=AsyncMock(return_value=fake_lake_session),
            ),
            patch(
                'src.engines.inventory_reconcile.actions.get_process_lake_catalog',
                return_value=fake_catalog,
            ),
            patch(
                'src.engines.inventory_reconcile.actions.get_process_lake_settings',
