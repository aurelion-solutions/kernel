...
from src.engines.inventory_reconcile.master_data_apply import MasterDataApplyResult
from src.engines.inventory_reconcile.master_data_pipeline import MasterDataReconciliationResult
from src.engines.inventory_reconcile.models import ReconciliationEntityType
from src.engines.inventory_reconcile.routes import router
...
async def test_trigger_master_data_run_returns_pending_apply(session_factory):
    """POST /inventory-reconciles/master-data/runs → 200 with status=pending_apply."""
    run_id = uuid.uuid4()
    ...
    with patch(
        'src.engines.inventory_reconcile.routes.run_master_data_reconciliation',
        new=AsyncMock(return_value=fake_pipeline_result),
    ):
        async with AsyncClient(transport=ASGITransport(app=app), base_url='http://test') as client:
            resp = await client.post(
                '/inventory-reconciles/master-data/runs',
...
async def test_trigger_master_data_run_does_not_call_apply(session_factory):
    ...
    with (
        ...
        patch(
            'src.engines.inventory_reconcile.routes.apply_master_data_delta',
            new=mock_apply,
        ),
    ):
        async with AsyncClient(transport=ASGITransport(app=app), base_url='http://test') as client:
...
async def test_apply_master_data_run_returns_200(session_factory):
    """POST /inventory-reconciles/master-data/runs/{id}/apply → 200."""
    run_id = uuid.uuid4()
    ...
    with patch(
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url='http://test') as client:
            resp = await client.post(
                f'/inventory-reconciles/master-data/runs/{run_id}/apply',
