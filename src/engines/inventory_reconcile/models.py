...
_run_status_enum = SaEnum(
    ...
    name='inventory_reconcile_run_status',
    create_type=True,
...
_delta_operation_enum = SaEnum(
    ...
    name='inventory_reconcile_delta_operation',
    create_type=True,
    values_callable=lambda x: [e.value for e in x],
)
...
_delta_item_status_enum = SaEnum(
    ...
    name='inventory_reconcile_delta_item_status',
    create_type=True,
    values_callable=lambda x: [e.value for e in x],
)
...
class ReconciliationEntityType(StrEnum):
    ...
    ``account``     — observed account data → accounts PG table.
    """
...
_entity_type_enum = SaEnum(
    ...
    name='inventory_reconcile_entity_type',
    create_type=True,
    values_callable=lambda x: [e.value for e in x],
)
...
class ReconciliationRun(Base):
    ...
    __tablename__ = 'inventory_reconcile_runs'
    ...
    __table_args__ = (
        sa.Index('ix_inventory_reconcile_runs_application_id', 'application_id'),
        sa.Index('ix_inventory_reconcile_runs_status', 'status'),
    )
...
class ReconciliationDeltaItem(Base):
    ...
    __tablename__ = 'inventory_reconcile_delta_items'
    ...
    reconciliation_run_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        sa.ForeignKey('inventory_reconcile_runs.id', ondelete='CASCADE'),
        nullable=False,
    )
    entity_type: Mapped[ReconciliationEntityType] = mapped_column(
        _entity_type_enum,
    ...
    __table_args__ = (
        ...
        # (Step 9 GET /inventory-reconciles/runs/{id}/delta-items and Step 12 apply-recovery scan).
