...
"""Tests for access_apply engine actions (Phase 19 Step A4, renamed from provisioning).

Covers:
- Registration of access_apply.create_account and access_apply.delete_account
...
- Side-effect import via src.engines.access_apply __init__.py.
"""
...
from src.engines.access_apply.actions import (
    CreateAccountArgs,
    CreateAccountResult,
    DeleteAccountArgs,
    DeleteAccountResult,
...
_ENGINE = 'access_apply'
_CREATE_ACTION = 'create_account'
_DELETE_ACTION = 'delete_account'
_ACTIONS_MODULE = 'src.engines.access_apply.actions'
...
def _registry_isolation() -> Iterator[None]:
    ...
    import src.engines.access_apply.tests.test_actions as _self_mod  # noqa: PLC0415
...
async def _seed_app_and_connector(session: AsyncSession) -> uuid.UUID:
    ...
    instance_id = f'aa-test-inst-{uuid.uuid4().hex[:8]}'
    await connector_service.upsert_instance(
        session,
        instance_id=instance_id,
        tags=['access-apply-test'],
    )

    app = Application(
        name=f'aa-test-app-{uuid.uuid4().hex[:8]}',
        code=f'aa-{uuid.uuid4().hex[:8]}',
        config={},
        required_connector_tags=['access-apply-test'],
        is_active=True,
    )
    session.add(app)
    await session.flush()
...
def test_create_account_action_registered() -> None:
    """access_apply.create_account is registered with idempotent=True and correct schemas."""
...
def test_delete_account_action_registered() -> None:
    """access_apply.delete_account is registered with idempotent=True and correct schemas."""
...
async def test_create_account_happy_path(session_factory) -> None:  # type: ignore[no-untyped-def]
    ...
    async with session_factory() as session:
        ...
        with patch(
            'src.engines.access_apply.actions._build_connector_client',
            return_value=dummy,
        ):
...
@pytest.mark.asyncio
async def test_access_apply_actions_do_not_commit_session(session_factory) -> None:  # type: ignore[no-untyped-def]
...
def test_slice_init_registers_actions() -> None:
    """Importing src.engines.access_apply triggers action registration."""
    ACTION_REGISTRY._clear_for_tests()
    # Remove cached modules so the import re-triggers side effects
    for mod in list(sys.modules):
        if mod.startswith('src.engines.access_apply'):
            sys.modules.pop(mod, None)

    importlib.import_module('src.engines.access_apply')
