...
"""HTTP tests for ``engines.access_apply.routes``."""
...
async def test_create_account_returns_201(session_factory):
    async with session_factory() as session:
        ...
        await connector_service.upsert_instance(
            session,
            instance_id='conn-inst-access-apply-1',
            tags=['jira', 'eu-segment'],
        )
        application = Application(
            name='access-apply-route-app-1',
            code='access-apply-route-app-1',
    ...
    test_app.include_router(access_apply_router, prefix='/api/v0')
...
async def test_create_account_returns_409_when_no_connector_instance_matches(
    ...
    async with session_factory() as session:
        application = Application(
            name='access-apply-route-app-2',
            code='access-apply-route-app-2',
            required_connector_tags=['jira', 'eu-segment'],
        )
        session.add(application)
        await session.commit()
